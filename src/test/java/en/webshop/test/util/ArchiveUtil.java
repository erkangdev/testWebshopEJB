...
public abstract class ArchiveUtil {
	...
	private static final String EJB_DIR = "../" + PROJEKT_NAME
			+ "EJB/build/classes";
	...
	private static EnterpriseArchive createTestArchive() {
		...
		/**
		 * Die Hilfsklassen DbReloadProvider und DbReload werden
		 * in den JUnit-Tests aufgerufen und muessen deshalb mitverpackt werden
		 * @return
		 */
		ejbJar.addClasses(DbReloadProvider.class, DbReload.class);
		ear.addModule(ejbJar);

		LOGGER.info(NEWLINE + ear.toString(true) + NEWLINE + NEWLINE
